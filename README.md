# work-shift-calendar

Статичний вебзастосунок для ведення змінного графіка, перегляду календаря, ручного коригування статусів днів, збереження нотаток і додаткових графіків.

Поточна версія застосунку: `1.0.43`.

## Можливості

- Готові цикли `5/5`, `4/4`, `5/2`, `3/3`, `2/2`
- Свій графік зі своїми робочими та вихідними днями
- Обов'язковий стартовий вибір графіка для нових користувачів
- Календар місяця зі статистикою періоду
- Ручна зміна статусу окремого дня
- Нотатки для конкретних дат
- Додаткові графіки з окремими кольорами, редагуванням і переглядом у календарі та модалці дня
- Позначення державних свят
- Прогноз погоди через `Open-Meteo` з автовизначенням геолокації (fallback — Бориспіль)
- Автооновлення погоди кожні 3 години
- Офлайн-режим — застосунок повністю доступний без інтернету до 7 днів (Service Worker)
- Збереження графіка, нотаток, ручних змін і вигляду в `localStorage`

## Поточна поведінка

- Якщо графік ще не налаштований, спочатку відкривається екран `Ваш графік`
- Поки користувач не обере графік і не застосує його, основний календар не показується
- Екран `Вигляд` відповідає за тему, кольори днів і системні візуальні дії
- Екран `Графіки` дозволяє додавати додаткові графіки, вмикати або ховати їх, редагувати окремо й вибирати колір із компактної м'якої палітри на 12 відтінків
- Режим теми `Авто` бере поточну тему пристрою через `prefers-color-scheme`
- Інтерфейс використовує один канонічний поточний стиль
- Верхній тулбар містить кнопки `Графіки / Ваш графік / Вигляд / тема` в одному icon-button патерні
- Основні дії зведені до спільної системи `icon / secondary / primary / danger`

## Оновлення інтерфейсу (квітень 2026)

- У верхньому тулбарі, перемиканні місяців і модальних вікнах емодзі-іконки замінені на єдиний SVG-набір для більш консистентного вигляду
- Для дій у профілях (`показати/сховати`, `редагувати`, `видалити`) додані чіткі `aria-label` і `aria-pressed`
- Колір профілю в формах створення/редагування має компактний режим із живим прев'ю прямо в тригері кнопки
- Для нових графіків колір за замовчуванням автоматично береться як наступний вільний із палітри, щоб уникати простих дублювань
- Активний фільтр у легенді календаря тепер керується через CSS-клас `is-active`, що спрощує стилізацію
- Свайп між місяцями став точнішим на мобільних: горизонтальний жест спрацьовує тільки коли явно домінує над вертикальним скролом
- У модалці дня назва свята показується без декоративного префікса, у більш нейтральному форматі
- Екрани `Графіки`, `Ваш графік` і `Вигляд` тепер працюють через спільний overlay-контролер, а скрол усередині панелей окремо підлаштований під мобільний touch і desktop mobile emulation

## Вигляд і скидання

У вікні `Вигляд` доступно:

- перемикання режиму теми: `Авто`, `Світла`, `Темна`
- зміна кольору робочих і вихідних днів
- `Скинути тему`, що скидає тему й кольори до базового вигляду
- повне очищення локальних даних через утримання кнопки `Повне очищення` протягом `1.5` секунд із додатковим підтвердженням

`Повне очищення` очищає:

- `scheduleConfig`
- `customDayStatuses`
- `dayNotes`
- `colleagueProfiles` з екрана `Графіки`
- візуальні налаштування теми й кольорів
- локальний кеш застосунку та тимчасові ключі оновлень

> Кеш свят **не** видаляється — свята з календаря залишаються після reset.

## Структура

```text
work-shift-calendar/
├── data/
├── js/
│   ├── calendar.js
│   ├── insights.js
│   ├── profiles-ui.js
│   ├── profiles.js
│   ├── schedule-ui.js
│   ├── schedule.js
│   ├── settings-state.js
│   ├── ui.js
│   ├── weather.js
│   └── ...
├── styles/
│   ├── calendar.css
│   ├── layout.css
│   ├── modal-day.css
│   ├── profiles.css
│   ├── responsive.css
│   ├── schedule-panel.css
│   ├── settings-panel.css
│   └── theme.css
├── sw.js
├── index.html
└── README.md
```

## Запуск

Для звичайного браузерного запуску окрема збірка не потрібна.

Рекомендований локальний режим розробки:

```bash
npm install
npm run dev:web
```

Для швидкої перевірки можна і без dev-сервера:

```bash
open index.html
```

Або:

- відкрити `index.html` напряму в браузері
- запустити через `Live Server` у редакторі

## Android через Capacitor

Проєкт підготовлено для запуску як Android-застосунок через `Capacitor`.

### Що вже додано

- `package.json` зі скриптами для підготовки веб-асетів
- `capacitor.config.json`
- Android-проєкт у папці `android/`
- проміжна папка `www/` генерується автоматично зі статичних файлів

### Встановлення

Потрібні:

- `Node.js 18+`
- `Java (JDK 17 рекомендовано)`
- `Android Studio`

Перший запуск:

```bash
npm install
npm run cap:android
npm run android:open
```

### Перший запуск в Android Studio

Для першої збірки краще спочатку відкрити Android Studio, а вже потім користуватись `gradlew`.

1. Встановіть `Android Studio` для `Apple Silicon`, якщо у вас Mac на `M1/M2/M3`
2. Під час першого запуску дозвольте встановити:
   - `Android SDK`
   - `Android SDK Platform`
   - `Android SDK Build-Tools`
   - `Android SDK Command-line Tools`
   - `Android Emulator`
3. Відкрийте Android-проєкт через:

```bash
npm run android:open
```

4. Дочекайтесь `Gradle Sync`
5. Після цього можна збирати `APK` як з Android Studio, так і з термінала

### Корисні команди

Зібрати веб-частину для Capacitor:

```bash
npm run build:web
```

Підняти локальний dev-сервер для live reload:

```bash
npm run dev:web
```

Запустити Android-застосунок на телефоні з live reload:

```bash
npm run android:live
```

Синхронізувати зміни в Android-проєкт:

```bash
npm run cap:android
```

Відкрити нативний Android-проєкт в Android Studio:

```bash
npm run android:open
```

### Швидкі візуальні правки без нового APK

Для правок `HTML/CSS/JS` не потрібно щоразу збирати новий `APK`.

1. Встановіть залежності:

```bash
npm install
```

2. У першому терміналі запустіть локальний сервер:

```bash
npm run dev:web
```

3. Підключіть телефон до тієї ж `Wi-Fi` мережі, що й комп'ютер.

4. У другому терміналі запустіть:

```bash
npm run android:live
```

Поведінка:

- застосунок відкриється на телефоні через локальний сервер
- зміни у `index.html`, `js/`, `styles/` і `data/` будуть підтягуватись без нового `APK`
- цей режим зручний саме для візуальних правок і швидкої перевірки UI
- для релізної збірки, змін у `AndroidManifest`, іконках, splash або іншому нативному коді все ще потрібен звичайний `cap:android` / новий білд

За потреби можна вручну задати IP або порт у `PowerShell`:

```bash
$env:CAP_LIVE_HOST="192.168.0.25"
$env:CAP_LIVE_PORT="4173"
npm run android:live
```

### Як далі збирати APK

Перед новою збіркою, якщо правка має піти на телефон як окрема версія, спочатку оновіть номер версії однією командою:

```bash
npm run version:set -- 1.0.1
```

Це синхронно оновлює:

- `package.json`
- `data/config.js`
- `android/app/build.gradle`

`beta/version.json` і `stable/version.json` оновлюються релізними workflow після тегів `beta-X.Y.Z` / `stable-X.Y.Z`.

1. Запустіть `npm run cap:android`
2. Відкрийте Android Studio через `npm run android:open`
3. Дочекайтесь Gradle Sync
4. Зберіть `APK` або `AAB` через меню `Build`

Або з термінала:

```bash
cd android
./gradlew assembleDebug
```

Готовий debug APK зазвичай буде тут:

```text
android/app/build/outputs/apk/debug/app-debug.apk
```

### Важливо

- папка `www/` не комітиться, вона генерується автоматично
- після змін у `index.html`, `js/`, `styles/` або `data/` потрібно знову запускати `npm run cap:android`
- перед кожним push / заливом у репозиторій оновлюйте `README.md` під актуальний стан функцій і релізного флоу
- для швидких UI-правок замість цього краще використовувати зв'язку `npm run dev:web` + `npm run android:live`
- локальні дані застосунку, як і раніше, зберігаються локально на пристрої
- починаючи з нових Android-білдів авто-відновлення даних після перевстановлення вимкнене, тому при чистому reinstall стартовий екран налаштування графіка з’являється знову
- вихідники для Android-іконки та splash лежать у `assets/android/`

### Перевірка нової версії APK

У застосунку є перевірка оновлень для Android:

- `stable` — основний канал для користувачів
- `beta` — внутрішній канал для тестових збірок

Короткий робочий флоу:

1. локально перевіряєте зміни через `npm run dev:web` + `npm run android:live`
2. перед новим білдом піднімаєте версію: `npm run version:set -- x.y.z`
3. тестову збірку публікуєте через `npm run release:beta -- x.y.z`
4. після перевірки публікуєте для всіх через `npm run release:stable -- x.y.z`

Маніфести оновлень живуть у `GitHub Pages`, а самі `APK` — у `GitHub Releases`.
Користувачу в релізі потрібно качати тільки файл `.apk` із секції **Assets**.

### Автоматичні релізи через GitHub Actions

У репозиторії є два workflow:

- `Release Beta APK` — збирає і публікує `beta`-APK
- `Promote Beta To Stable` — публікує перевірений білд у `stable`

Окремо preflight можна прогнати вручну:

```bash
npm run release:check -- beta x.y.z
```

Для release-білдів потрібні GitHub Secrets:

- `ANDROID_KEYSTORE_BASE64`
- `ANDROID_KEYSTORE_PASSWORD`
- `ANDROID_KEY_ALIAS`
- `ANDROID_KEY_PASSWORD`

## GitHub Pages

У репозиторії є workflow для публікації через `GitHub Pages` з гілки `main`.

- після `push` у `main` сайт оновлюється автоматично
- у налаштуваннях репозиторію для `Pages` має бути вибране джерело `GitHub Actions`
- якщо `beta/version.json` або `stable/version.json` були змінені самим GitHub Actions workflow, а сайт ще показує старий маніфест, потрібно дати окремий user-push у `main` або вручну перезапустити `Deploy GitHub Pages`

## Як користуватись

1. При першому запуску виберіть свій графік і дату початку циклу.
2. Переглядайте місяці кнопками навігації.
3. Натискайте на день, щоб відкрити деталі, змінити статус або додати нотатку.
4. Відкрийте `Ваш графік`, щоб змінити цикл або дату старту.
5. Відкрийте `Графіки`, щоб додати або відредагувати додаткові графіки.
6. Відкрийте `Вигляд`, щоб змінити тему й кольори днів.

## Примітки

- погода автоматично визначає геолокацію при відкритті застосунку; якщо доступ заборонено — використовується Бориспіль як fallback
- погода оновлюється автоматично раз на 3 години, поки застосунок відкритий
- Service Worker кешує всі статичні ресурси і забезпечує офлайн-роботу до 7 днів; API-запити (погода, свята) кешуються з тим самим TTL
- без інтернету календар, графік, нотатки, свята і весь інтерфейс працюють далі
- застосунок побудований на `HTML`, `CSS` і `Vanilla JavaScript`
